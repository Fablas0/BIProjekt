...
def zeichne() -> None:
    ...
    with st.sidebar:
        ...
        mindest_nutzung = st.slider(
            "Mindest-Nutzungsanteil (%)", 0.0, 20.0, 0.0, 0.5,
            help="Blendet Nischen-Pokemon aus und schaerft dadurch die Verdichtung.",
        )

    teilwuerfel = olap.dice_wuerfel(wuerfel, bedingungen)
    if mindest_nutzung > 0:
        teilwuerfel = teilwuerfel[teilwuerfel["usage_rate"] >= mindest_nutzung]
    ...
    spalten[1].markdown(kennzahl_kachel("Pokemon", str(eckwerte["pokemon"]),
                        ...
                        unsafe_allow_html=True)
    spalten[2].markdown(kennzahl_kachel("Monate", str(eckwerte["monate"]),
                                        "Zeitdimension", "#EE8130"), unsafe_allow_html=True)
    spalten[3].markdown(kennzahl_kachel("Summe Nutzung", f"{eckwerte['usage_summe']:.0f} %",
                                        "aufsummiert ueber den Teilwuerfel", "#A33EA1"),
                        unsafe_allow_html=True)
