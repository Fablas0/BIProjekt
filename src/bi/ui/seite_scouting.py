...
def zeichne() -> None:
    ...
    with kopf_rechts:
        monat = monatsauswahl(monate, "scouting_monat")
    ...
    with kopf_links:
        auswahl = st.multiselect(
            ...
            max_selections=6,
            help="Die Auswahl umfasst alle Pokemon, die im gewaehlten Monat "
                 "einen messbaren Nutzungsanteil hatten.",
        )
    ...
    _zeige_teamuebersicht(conn, team, monat)
    ...
    with reiter[0]:
        _zeige_konfiguration(conn, team, monat)
    with reiter[1]:
        _zeige_strategie(conn, auswahl, monat)
    with reiter[2]:
        _zeige_schwaechen(conn, team, monat)
    with reiter[3]:
        _zeige_bedrohungen(conn, team, monat)
    with reiter[4]:
...
def _zeige_meta_einstieg(verfuegbar: pd.DataFrame) -> None:
    ...
    st.markdown("#### Haeufigste Pokemon im gewaehlten Monat")
    ...
    for spalte, (_, zeile) in zip(spalten, verfuegbar.head(6).iterrows(), strict=False):
        with spalte:
            st.markdown(
                ...
                f"<div style='font-size:0.75rem;opacity:0.7;'>{zeile['usage_rate']:.1f} %</div>"
                f"</div>", unsafe_allow_html=True,
            )


def _zeige_teamuebersicht(conn, team: pd.DataFrame, monat: str) -> None:
    ...
    for spalte, (_, zeile) in zip(spalten, team.iterrows(), strict=True):
        set_daten = kpi.standardset(conn, zeile["anzeigename"], monat)
        ...
        top_tera = set_daten["tera"].iloc[0] if not set_daten["tera"].empty else None
        ...
        with spalte:
            st.markdown(
                ...
                f"<div style='font-size:0.78rem;opacity:0.75;'>Nutzung {zeile['usage_rate']:.1f} % "
                f"· Rang {int(zeile['rang'])}</div></div>",
                unsafe_allow_html=True,
            )
            if top_faehigkeit is not None:
                st.markdown(f"**Faehigkeit** · {top_faehigkeit['auspraegung']} "
                            f"({top_faehigkeit['anteil']:.0f} %)")
            if top_item is not None:
                st.markdown(f"**Item** · {top_item['auspraegung']} ({top_item['anteil']:.0f} %)")
            if top_tera is not None:
                st.markdown(f"**Tera** · {TYP_DEUTSCH.get(top_tera['auspraegung'], top_tera['auspraegung'])} "
                            f"({top_tera['anteil']:.0f} %)")

            attacken = set_daten["attacken"].head(4)
            if not attacken.empty:
                zeilen = "".join(
                    f"<div>{a['attacke']} <span style='opacity:0.6;'>{a['anteil']:.0f} %</span></div>"
...
def _zeige_konfiguration(conn, team: pd.DataFrame, monat: str) -> None:
    ...
    for _, zeile in team.iterrows():
        set_daten = kpi.standardset(conn, zeile["anzeigename"], monat)
        with st.expander(f"{zeile['anzeigename']} · Nutzung {zeile['usage_rate']:.1f} %",
                         expanded=len(team) <= 2):
            ...
            with links:
                ...
                if attacken.empty:
                    ...
                    abbildung = px.bar(
                        attacken.sort_values("anteil"), x="anteil", y="attacke",
                        orientation="h", color="taktik_klasse",
                        labels={"anteil": "Anteil der Sets (%)", "attacke": "",
            ...
            with mitte:
                st.markdown("**Items**")
                for _, i in set_daten["items"].iterrows():
                    st.markdown(f"{i['auspraegung']} · **{i['anteil']:.0f} %**")

            with rechts:
                st.markdown("**Tera-Typen**")
                for _, t in set_daten["tera"].iterrows():
                    typ = TYP_DEUTSCH.get(t["auspraegung"], t["auspraegung"])
                    st.markdown(f"{typ} · **{t['anteil']:.0f} %**")


def _zeige_strategie(conn, namen: list[str], monat: str) -> None:
    """Strategie-Radar auf Basis der angereicherten Taktik-Klassen."""
    befunde = threat.strategie_radar(conn, namen, monat)
...
def _zeige_schwaechen(conn, team: pd.DataFrame, monat: str) -> None:
    ...
    st.markdown(
        ...
        "Anfaelligkeiten mit der aus der Faktentabelle ermittelten Haeufigkeit des "
...
def _zeige_bedrohungen(conn, team: pd.DataFrame, monat: str) -> None:
    ...
    bedrohungen = threat.bedrohungsindex(conn, team, monat, grenze=10)
    ...
    st.dataframe(
        bedrohungen.rename(columns={
            ...
            "usage_rate": "Nutzung (%)", "bedrohungswert": "Bedrohungswert",
            "anzahl_gefaehrdet": "Gefaehrdete Mitglieder", "beste_attacke": "Gefaehrlichste Attacke",
            "gefaehrdet": "Betroffen",
        })[["Pokemon", "Typen", "Nutzung (%)", "Bedrohungswert", "Gefaehrlichste Attacke",
