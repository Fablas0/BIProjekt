...
# --------------------------------------------------------------------------
# Schreibweise des Quellsystems Pokemon Champions
#
# Champions benennt Formen ausgeschrieben und stellt die Region voran
# ("Alolan Ninetales", "Aegislash Shield Forme"), waehrend die PokeAPI sie
# anhaengt ("ninetales-alola", "aegislash-shield"). Statt alle Formen einzeln zu
# hinterlegen, sind die zugrundeliegenden Regeln abgebildet -- kuenftige Formen
# desselben Musters werden dadurch ohne Codeaenderung erkannt.
# --------------------------------------------------------------------------

# Vorangestellte Region -> angehaengtes Suffix der PokeAPI.
REGIONEN = {
    "alolan": "alola",
    "galarian": "galar",
    "hisuian": "hisui",
...
# Rotom-Formen werden als "<Form> Rotom" gefuehrt.
ROTOM_FORMEN = {"heat", "wash", "frost", "fan", "mow"}

# Fuellwoerter am Ende einer Formbezeichnung, die die PokeAPI nicht kennt.
FORM_FUELLWOERTER = ("forme", "form", "variety", "pattern", "flower", "plumage", "style")

# Formen, deren Bezeichnung sich nicht regelbasiert ableiten laesst.
CHAMPIONS_SONDERFAELLE = {
    "gourgeist jumbo variety": "gourgeist-super",
    "pumpkaboo jumbo size": "pumpkaboo-super",
}


def _champions_kandidaten(bezeichner: str) -> list[str]:
    """Erzeugt moegliche PokeAPI-Slugs aus einer Champions-Bezeichnung."""
    roh = bezeichner.strip().lower()
    if roh in CHAMPIONS_SONDERFAELLE:
        return [CHAMPIONS_SONDERFAELLE[roh]]
    ...
    # "<Form> Rotom" -> "rotom-<form>"
    if len(woerter) == 2 and woerter[1] == "rotom" and woerter[0] in ROTOM_FORMEN:
        return [f"rotom-{woerter[0]}"]

    region = REGIONEN.get(woerter[0])
    if region:
        rest = woerter[1:]
        # "Paldean Tauros Aqua Breed" -> "tauros-paldea-aqua-breed":
        # die Region wird hinter den Artnamen gesetzt, der Rest bleibt stehen.
        if len(rest) > 1:
            kandidaten.append(normalisiere(" ".join([rest[0], region, *rest[1:]])))
        kandidaten.append(normalisiere(" ".join([*rest, region])))

    # Fuellwoerter am Ende abschneiden, schrittweise von hinten.
    teile = list(woerter)
    while teile and teile[-1] in FORM_FUELLWOERTER:
        teile.pop()
        if teile:
            kandidaten.append(normalisiere(" ".join(teile)))
    # Auch der reine Artname ohne jede Formangabe ist ein Kandidat --
    # kosmetische Formen fuehrt die PokeAPI nicht getrennt.
    if teile:
        kandidaten.append(normalisiere(teile[0]))
...
def loese_auf(quell_name: str, bekannte_slugs: set[str]) -> str | None:
    ...
    # 5. Schreibweise des Quellsystems Pokemon Champions.
    for kandidat in _champions_kandidaten(quell_name):
        if kandidat in bekannte_slugs:
            return kandidat
